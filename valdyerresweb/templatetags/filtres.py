...
def dateCustom(debutUTC, finUTC):
    ...
    delta = fin - debut
    if delta.days >= 1:
        text = u"du " + jours[int(debut.strftime("%w"))] + u" " + debut.strftime(u"%d") + u" " + mois[
            int(debut.strftime(u"%m")) - 1] + u" au " + jours[int(fin.strftime(u"%w"))] + u" " + fin.strftime(
            u"%d") + u" " + mois[int(fin.strftime(u"%m")) - 1]
    else:
        ...
        if deltanow.days == 0:
            ...
            text = jours[int(debut.strftime(u"%w"))] + u" " + debut.strftime(u"%d") + u" " + mois[
                int(debut.strftime(u"%m")) - 1] + u" à " + debut.strftime(u"%H:%M")
    return text
...
def dateSimple(mydate):
    ...
    text = jours[int(thedate.strftime(u"%w"))] + u" " + thedate.strftime(u"%d") + u" " + mois[
        int(thedate.strftime(u"%m")) - 1] + " " + thedate.strftime(u"%Y") + u" à " + thedate.strftime(u"%H:%M")
    return text


@register.filter(is_safe=True)
def dateNewsletter(debutUTC, finUTC):
    TZone = timezone(settings.TIME_ZONE)
    debut = debutUTC.astimezone(TZone)
    fin = finUTC.astimezone(TZone)
    now = datetime.datetime.now(TZone)
    ...
    if delta.days >= 1:
        ...
        text = jours[int(debut.strftime(u"%w"))] + u" " + debut.strftime(u"%d") + u" " + mois[
            int(debut.strftime(u"%m")) - 1] + u" <br/> " + debut.strftime(u"%H:%M") + u"-" + fin.strftime(u"%H:%M")
    return text
...
def dateFormat(dateUTC):
    ...
    return date.strftime("%Y") + date.strftime("%m") + date.strftime("%d") + "T" + date.strftime("%H") + date.strftime(
        "%M") + date.strftime("%S")
...
def dateSEO(dateUTC):
    TZone = timezone(settings.TIME_ZONE)
    date = dateUTC.astimezone(TZone)
    return date.strftime("%Y") + "-" + date.strftime("%m") + "-" + date.strftime("%d") + "T" + date.strftime(
        "%H") + ":" + date.strftime("%M")
    ...
    try:
        ...
        try:
            path = myfile.path.replace(settings.MEDIA_ROOT,
                                       "")  # TODO: trouver pkoi Image et Filebrowsefield renvoient des chemins différents
